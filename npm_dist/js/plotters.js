...
var Plotter = exports.Plotter = function (_NamedObject) {
    ...
    _createClass(Plotter, null, [{
        ...
        value: function createRectangles(context, rects, sign) {
            context.beginPath();
            var e = void 0,
                i = void 0,
                cnt = rects.length;
...
var RangeAreaBackgroundPlotter = exports.RangeAreaBackgroundPlotter = function (_BackgroundPlotter2) {
    ...
    _createClass(RangeAreaBackgroundPlotter, [{
        ...
        value: function Draw(context) {
            ...
            if (!isMainRange && !area.isChanged() && !range.isUpdated() && areaName !== "frame0.k0.indic0Range") {
                return;
            }
            var theme = mgr.getTheme(this.getFrameName());
            context.fillStyle = theme.getColor(this._color);
...
var CandlestickPlotter = exports.CandlestickPlotter = function (_NamedObject3) {
    ...
    _createClass(CandlestickPlotter, [{
        ...
        value: function Draw(context) {
            ...
            var coordinates = _kline2.default.instance.data.coordinates;
            var monthCoordinates = _kline2.default.instance.data.monthCoordinates;

            function drawTrendLine(ctx, x1, x2, y1, y2, color) {
                ...
                // context.fillText(yn.toFixed(2), x2-30, y-7);
            }
            var colorList = ['#ffa39e', '#ff9c6e', '#ffc069', '#ffc53d', '#fadb14', '#7cb305', '#096dd9', '#08979c', '#389e0d'];
            if (coordinates && tmp.indics['TREND'] && tmp.indics['TREND'][0] !== 0) {
                for (var i = 0; i < coordinates.length; i++) {
                    var item = coordinates[i];
                    drawTrendLine(context, timeline.toItemCenter(item.high_time_key1), timeline.toItemCenter(item.high_time_key2), range.toY(item.position_high1), range.toY(item.position_high2), '#189fff');
                    drawTrendLine(context, timeline.toItemCenter(item.low_time_key1), timeline.toItemCenter(item.low_time_key2), range.toY(item.position_low1), range.toY(item.position_low2), '#189fff');
                }
            }
            if (monthCoordinates && tmp.indics['TREND'] && tmp.indics['TREND'][1] !== 0) {
                drawTrendLine(context, timeline.toItemCenter(monthCoordinates.high_time_key_month1), timeline.toItemCenter(monthCoordinates.high_time_key_month2), range.toY(monthCoordinates.position_high_month1), range.toY(monthCoordinates.position_high_month2), '#389e0d');
                drawTrendLine(context, timeline.toItemCenter(monthCoordinates.low_time_key_month1), timeline.toItemCenter(monthCoordinates.low_time_key_month2), range.toY(monthCoordinates.position_low_month1), range.toY(monthCoordinates.position_low_month2), '#389e0d');
            }

            //标线
            var signLine = [];
            var closeNumber = ds.getDataAt(ds._dataItems.length - 1).close;
            if (range.toY(closeNumber * 0.85) > area.getTop()) {
                var drawLineDash = function drawLineDash(ctx, x1, x2, y, yn, value) {
                    ctx.setLineDash([5, 10]);
                    ctx.lineWidth = 2;
                    ctx.strokeStyle = 'rgba(144,144,144,0.6)';
                    ctx.beginPath();
                    ctx.moveTo(x1, y);
                    ctx.lineTo(x2, y);
                    ctx.stroke();
                    ...
                    context.textBaseline = "middle";
                    // 绘制文字（参数：要写的字，x坐标，y坐标）
                    context.fillText(value || '' + yn.toFixed(2), x2 - 30, y - 7);
                    context.setLineDash([]);
                };

                signLine.push({ x1: area.getLeft(), x2: area.getRight(), y: range.toY(closeNumber * 0.9), yn: closeNumber * 0.9, value: '9折线' });
                signLine.push({ x1: area.getLeft(), x2: area.getRight(), y: range.toY(closeNumber * 0.85), yn: closeNumber * 0.85, value: '85折线' });

                drawLineDash(context, signLine[0].x1, signLine[0].x2, signLine[0].y, signLine[0].yn);
                drawLineDash(context, signLine[1].x1, signLine[1].x2, signLine[1].y, signLine[1].yn);
            }

            for (var _i2 = start; _i2 < last; _i2++) {
                var data = ds.getDataAt(_i2);
                ...
                //标记
                if (data.sign === 1) {
                    sign.push({ x: center + 0.5, y: high - 4, w: Math.max(iW / 2, 2), direction: 1 });
                } else if (data.sign === 0) {
                    sign.push({ x: center + 0.5, y: low + 4, w: Math.max(iW / 2, 2), direction: 0 });
                }
                ...
                // sign.push({ x: center, y: high-30, w: 1, h: 20 });
                left += cW;
                center += cW;
            }

            function GreenSign(ctx, dx, dy, iw) {
                ctx.beginPath();
                ctx.fillStyle = "rgba(0,155,0,0.9)";
                ctx.moveTo(dx, dy);
                ctx.lineTo(dx + iw, dy - iw);
                ctx.lineTo(dx + iw / 3, dy - iw);
                ctx.lineTo(dx + iw / 3, dy - iw * 2);
                ctx.lineTo(dx - iw / 3, dy - iw * 2);
                ctx.lineTo(dx - iw / 3, dy - iw);
                ctx.lineTo(dx - iw, dy - iw);
                ctx.lineTo(dx, dy);
            ...
            if (strokePosRects.length > 0) {
                ...
                Plotter.createRectangles(context, strokePosRects, sign);
                context.stroke();
            }
            if (fillPosRects.length > 0) {
                context.fillStyle = theme.getColor(themes.Theme.Color.Positive);
                Plotter.createRectangles(context, fillPosRects, sign);
                context.fill();
                for (var i = 0; i < sign.length; i++) {
                    if (sign[i].direction === 1) {
                        GreenSign(context, sign[i].x, sign[i].y, sign[i].w);
                    } else {
                        RedSign(context, sign[i].x, sign[i].y, sign[i].w);
                    }
                }
            }
            if (fillUchRects.length > 0) {
                context.fillStyle = theme.getColor(themes.Theme.Color.Negative);
                Plotter.createRectangles(context, fillUchRects, sign);
                context.fill();
                for (var i = 0; i < sign.length; i++) {
                    if (sign[i].direction === 1) {
                        GreenSign(context, sign[i].x, sign[i].y, sign[i].w);
            ...
            if (fillNegRects.length > 0) {
                context.fillStyle = theme.getColor(themes.Theme.Color.Negative);
                Plotter.createRectangles(context, fillNegRects, sign);
                context.fill();
            }
        }
    }]);
...
// export class AssistPlotter extends NamedObject {
//
//     constructor(name) {
//         super(name);
//     }
//
//     Draw() {
//         /*{"position_high1":166.69,"position_high2":145.82,"position_low1":155.45,"position_low2":134.87,
//         "high_time_key1":1537934400000,"high_time_key2":1540267200000,"low_time_key1":1538366400000,"low_time_key2":1540353600000},*/
...
//
//         /* if(range.toY(closeNumber*0.85) > area.getTop()){
//              signLine.push({x1:area.getLeft(), x2:area.getRight(), y: range.toY(closeNumber*0.9), yn:closeNumber*0.9});
//              signLine.push({x1:area.getLeft(), x2:area.getRight(), y: range.toY(closeNumber*0.85), yn:closeNumber*0.85});
//              function drawLineDash(ctx, x1, x2, y, yn) {
//                  ctx.setLineDash([5,10]);
//                  ctx.lineWidth = 2;
//                  ctx.strokeStyle = 'rgba(144,144,144,0.6)';
//                  ctx.beginPath();
...
var MainInfoPlotter = exports.MainInfoPlotter = function (_Plotter4) {
    ...
    _createClass(MainInfoPlotter, [{
        ...
        value: function Draw(context) {
            ...
            if (!Plotter.drawString(context, '  时间: ' + year + '-' + month + '-' + date + " ", rect)) {
                return;
            }
            rect.y = rect.y + 20;
            rect.x = area.getLeft() + 4;
            if (lang === "zh-cn") {
...
var IndicatorPlotter2 = exports.IndicatorPlotter2 = function (_NamedObject5) {
    _inherits(IndicatorPlotter2, _NamedObject5);

    function IndicatorPlotter2(name) {
        _classCallCheck(this, IndicatorPlotter2);

        return _possibleConstructorReturn(this, (IndicatorPlotter2.__proto__ || Object.getPrototypeOf(IndicatorPlotter2)).call(this, name));
    }

    _createClass(IndicatorPlotter2, [{
        key: 'Draw',
        value: function Draw(context) {
            var mgr = _chart_manager.ChartManager.instance;
            var area = mgr.getArea(this.getAreaName());
        ...
        value: function drawMACDStick(context, theme, output, first, last, startX, cW, iW, range) {
            ...
            for (var i = first; i < last; i++) {
                ...
                if (MACD >= 0) {
                    ...
                    var _iH4 = range.toHeight(-MACD);
                    if ((i === 0 || MACD >= prevMACD) && _iH4 > 1 && iW > 1) strokeNegRects.push({ x: left + 0.5, y: middle + 0.5, w: iW - 1, h: _iH4 - 1 });else fillNegRects.push({ x: left, y: middle, w: Math.max(iW, 1), h: Math.max(_iH4, 1) });
                }
                prevMACD = MACD;
                left += cW;
            }
...
var MinMaxPlotter = exports.MinMaxPlotter = function (_NamedObject6) {
    _inherits(MinMaxPlotter, _NamedObject6);
...
var RangePlotter = exports.RangePlotter = function (_NamedObject7) {
    _inherits(RangePlotter, _NamedObject7);
    ...
    _createClass(RangePlotter, [{
        ...
        value: function Draw(context) {
            ...
            var isMainRange = range.getNameObject().getCompAt(2) === "main";
            if (isMainRange) {} else {
                if (!area.isChanged() && !range.isUpdated() && areaName !== "frame0.k0.indic0Range") return;
            }
            var gradations = range.getGradations();
...
var COrderGraphPlotter = exports.COrderGraphPlotter = function (_NamedObject8) {
    _inherits(COrderGraphPlotter, _NamedObject8);
    ...
    _createClass(COrderGraphPlotter, [{
        ...
        value: function DrawLine(context) {
            ...
            if (this.m_mode === 0 || this.m_mode === 2) {
                ...
                for (var _i3 = 1; _i3 < this.m_bid_points.length; _i3++) {
                    context.lineTo(this.m_bid_points[_i3].x + 0.5, this.m_bid_points[_i3].y + 0.5);
                }
                context.stroke();
            }
        }
        ...
        value: function UpdatePoints() {
            ...
            var bid_p_i = 0;
            for (var _i4 = this.m_bid_si; _i4 <= this.m_bid_ei; _i4++) {
                var _index = {};
                var _index2 = {};
                if (_i4 === this.m_bid_si) {
                    _index.x = Math.floor(this.m_left + all.array[_i4].amounts * this.m_Step + this.x_offset);
                    _index.y = Math.ceil(this.m_pRange.toY(all.array[_i4].rate) + this.y_offset);
                    ...
                    _index2.y = Math.ceil(this.m_pRange.toY(all.array[_i4].rate) + this.x_offset);
                    this.m_bid_points.push(_index);
                    bid_p_i++;
        ...
        value: function updateData() {
            ...
            for (var _i5 = all.bids_si; _i5 <= all.bids_ei; _i5++) {
                if (all.array[_i5].rate > minRange) this.m_bid_ei = _i5;else break;
            }
        ...
        value: function FillBlack(context) {
            ...
            for (var _i6 = 1; _i6 < bid_point.length; _i6++) {
                context.lineTo(Math.floor(bid_point[_i6].x) + 0.5, Math.floor(bid_point[_i6].y) + 0.5);
            }
            context.fill();
            ask_point.shift();
            ask_point.pop();
...
var RangeSelectionPlotter = exports.RangeSelectionPlotter = function (_NamedObject9) {
    _inherits(RangeSelectionPlotter, _NamedObject9);
...
var CToolPlotter = exports.CToolPlotter = function (_NamedObject10) {
    _inherits(CToolPlotter, _NamedObject10);

    function CToolPlotter(name, toolObject) {
        _classCallCheck(this, CToolPlotter);

        var _this23 = _possibleConstructorReturn(this, (CToolPlotter.__proto__ || Object.getPrototypeOf(CToolPlotter)).call(this, name));
        ...
        _this23.crossPt = {};
        _this23.normalSize = 4;
        _this23.selectedSize = 6;
        _this23.cursorLen = 4;
        _this23.cursorGapLen = 3;
        _this23.theme = _chart_manager.ChartManager.instance.getTheme(_this23.getFrameName());
        return _this23;
    }
...
var DrawStraightLinesPlotter = exports.DrawStraightLinesPlotter = function (_CToolPlotter) {
    ...
    function DrawStraightLinesPlotter(name, toolObject) {
        ...
        var _this24 = _possibleConstructorReturn(this, (DrawStraightLinesPlotter.__proto__ || Object.getPrototypeOf(DrawStraightLinesPlotter)).call(this, name, toolObject));
...
var DrawSegLinesPlotter = exports.DrawSegLinesPlotter = function (_CToolPlotter2) {
    ...
    function DrawSegLinesPlotter(name, toolObject) {
        ...
        var _this25 = _possibleConstructorReturn(this, (DrawSegLinesPlotter.__proto__ || Object.getPrototypeOf(DrawSegLinesPlotter)).call(this, name, toolObject));
...
var DrawRayLinesPlotter = exports.DrawRayLinesPlotter = function (_CToolPlotter3) {
    ...
    function DrawRayLinesPlotter(name, toolObject) {
        ...
        var _this26 = _possibleConstructorReturn(this, (DrawRayLinesPlotter.__proto__ || Object.getPrototypeOf(DrawRayLinesPlotter)).call(this, name));
...
var DrawArrowLinesPlotter = exports.DrawArrowLinesPlotter = function (_CToolPlotter4) {
    ...
    function DrawArrowLinesPlotter(name, toolObject) {
        ...
        var _this27 = _possibleConstructorReturn(this, (DrawArrowLinesPlotter.__proto__ || Object.getPrototypeOf(DrawArrowLinesPlotter)).call(this, name, toolObject));

        _this27.toolObject = toolObject;
        _this27.arrowSizeRatio = 0.03;
        _this27.arrowSize = 4;
        _this27.crossPt = { x: -1, y: -1 };
        _this27.ctrlPtsNum = 2;
...
var DrawHoriStraightLinesPlotter = exports.DrawHoriStraightLinesPlotter = function (_CToolPlotter5) {
    ...
    function DrawHoriStraightLinesPlotter(name, toolObject) {
        ...
        var _this28 = _possibleConstructorReturn(this, (DrawHoriStraightLinesPlotter.__proto__ || Object.getPrototypeOf(DrawHoriStraightLinesPlotter)).call(this, name));

        _this28.toolObject = toolObject;
        _this28.ctrlPtsNum = 1;
...
var DrawHoriRayLinesPlotter = exports.DrawHoriRayLinesPlotter = function (_CToolPlotter6) {
    ...
    function DrawHoriRayLinesPlotter(name, toolObject) {
        ...
        var _this29 = _possibleConstructorReturn(this, (DrawHoriRayLinesPlotter.__proto__ || Object.getPrototypeOf(DrawHoriRayLinesPlotter)).call(this, name));
...
var DrawHoriSegLinesPlotter = exports.DrawHoriSegLinesPlotter = function (_CToolPlotter7) {
    ...
    function DrawHoriSegLinesPlotter(name, toolObject) {
        ...
        var _this30 = _possibleConstructorReturn(this, (DrawHoriSegLinesPlotter.__proto__ || Object.getPrototypeOf(DrawHoriSegLinesPlotter)).call(this, name, toolObject));

        _this30.toolObject = toolObject;
        _this30.ctrlPtsNum = 2;
...
var DrawVertiStraightLinesPlotter = exports.DrawVertiStraightLinesPlotter = function (_CToolPlotter8) {
    ...
    function DrawVertiStraightLinesPlotter(name, toolObject) {
        ...
        var _this31 = _possibleConstructorReturn(this, (DrawVertiStraightLinesPlotter.__proto__ || Object.getPrototypeOf(DrawVertiStraightLinesPlotter)).call(this, name));
...
var DrawPriceLinesPlotter = exports.DrawPriceLinesPlotter = function (_CToolPlotter9) {
    ...
    function DrawPriceLinesPlotter(name, toolObject) {
        ...
        var _this32 = _possibleConstructorReturn(this, (DrawPriceLinesPlotter.__proto__ || Object.getPrototypeOf(DrawPriceLinesPlotter)).call(this, name));

        _this32.toolObject = toolObject;
        _this32.ctrlPtsNum = 1;
        _this32.ctrlPts = [new Array(_this32.ctrlPtsNum), new Array(2)];
        _this32.getCtrlPts();
        return _this32;
    }
...
var ParallelLinesPlotter = exports.ParallelLinesPlotter = function (_CToolPlotter10) {
    ...
    function ParallelLinesPlotter(name, toolObject) {
        ...
        var _this33 = _possibleConstructorReturn(this, (ParallelLinesPlotter.__proto__ || Object.getPrototypeOf(ParallelLinesPlotter)).call(this, name));
...
var DrawBiParallelLinesPlotter = exports.DrawBiParallelLinesPlotter = function (_ParallelLinesPlotter) {
    ...
    function DrawBiParallelLinesPlotter(name, toolObject) {
        ...
        var _this34 = _possibleConstructorReturn(this, (DrawBiParallelLinesPlotter.__proto__ || Object.getPrototypeOf(DrawBiParallelLinesPlotter)).call(this, name, toolObject));
...
var DrawBiParallelRayLinesPlotter = exports.DrawBiParallelRayLinesPlotter = function (_ParallelLinesPlotter2) {
    ...
    function DrawBiParallelRayLinesPlotter(name, toolObject) {
        ...
        var _this35 = _possibleConstructorReturn(this, (DrawBiParallelRayLinesPlotter.__proto__ || Object.getPrototypeOf(DrawBiParallelRayLinesPlotter)).call(this, name, toolObject));
...
var DrawTriParallelLinesPlotter = exports.DrawTriParallelLinesPlotter = function (_ParallelLinesPlotter3) {
    ...
    function DrawTriParallelLinesPlotter(name, toolObject) {
        ...
        var _this36 = _possibleConstructorReturn(this, (DrawTriParallelLinesPlotter.__proto__ || Object.getPrototypeOf(DrawTriParallelLinesPlotter)).call(this, name, toolObject));

        _this36.toolObject = toolObject;
        _this36.ctrlPtsNum = 3;
...
var BandLinesPlotter = exports.BandLinesPlotter = function (_CToolPlotter11) {
    ...
    function BandLinesPlotter(name, toolObject) {
        ...
        var _this37 = _possibleConstructorReturn(this, (BandLinesPlotter.__proto__ || Object.getPrototypeOf(BandLinesPlotter)).call(this, name));

        _this37.toolObject = toolObject;
        _this37.ctrlPtsNum = 2;
        _this37.ctrlPts = [new Array(_this37.ctrlPtsNum), new Array(2)];
        _this37.getCtrlPts();
        return _this37;
    }
...
var DrawFibRetracePlotter = exports.DrawFibRetracePlotter = function (_BandLinesPlotter) {
    ...
    function DrawFibRetracePlotter(name, toolObject) {
        ...
        var _this38 = _possibleConstructorReturn(this, (DrawFibRetracePlotter.__proto__ || Object.getPrototypeOf(DrawFibRetracePlotter)).call(this, name, toolObject));

        _this38.toolObject = toolObject;
        _this38.fiboSequence = [100.0, 78.6, 61.8, 50.0, 38.2, 23.6, 0.0];
        return _this38;
    }
...
var DrawBandLinesPlotter = exports.DrawBandLinesPlotter = function (_BandLinesPlotter2) {
    ...
    function DrawBandLinesPlotter(name, toolObject) {
        ...
        var _this39 = _possibleConstructorReturn(this, (DrawBandLinesPlotter.__proto__ || Object.getPrototypeOf(DrawBandLinesPlotter)).call(this, name, toolObject));

        _this39.toolObject = toolObject;
        _this39.fiboSequence = [0, 12.5, 25, 37.5, 50, 62.5, 75, 87.5, 100];
        return _this39;
    }
...
var DrawFibFansPlotter = exports.DrawFibFansPlotter = function (_CToolPlotter12) {
    ...
    function DrawFibFansPlotter(name, toolObject) {
        ...
        var _this40 = _possibleConstructorReturn(this, (DrawFibFansPlotter.__proto__ || Object.getPrototypeOf(DrawFibFansPlotter)).call(this, name));

        _this40.toolObject = toolObject;
        _this40.fiboFansSequence = [0, 38.2, 50, 61.8];
        _this40.ctrlPtsNum = 2;
        _this40.ctrlPts = [new Array(_this40.ctrlPtsNum), new Array(2)];
        _this40.getCtrlPts();
        return _this40;
    }
...
var CDynamicLinePlotter = exports.CDynamicLinePlotter = function (_NamedObject11) {
    _inherits(CDynamicLinePlotter, _NamedObject11);

    function CDynamicLinePlotter(name) {
        _classCallCheck(this, CDynamicLinePlotter);

        var _this41 = _possibleConstructorReturn(this, (CDynamicLinePlotter.__proto__ || Object.getPrototypeOf(CDynamicLinePlotter)).call(this, name));

        _this41.flag = true;
        _this41.context = _chart_manager.ChartManager.instance._overlayContext;
        return _this41;
    }
