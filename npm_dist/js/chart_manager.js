...
var ChartManager = exports.ChartManager = function () {
    ...
    _createClass(ChartManager, [{
        ...
        value: function bindCanvas(layer, canvas) {
            if (layer === "main") {
                ...
                // 浏览器在渲染canvas之前存储画布信息的像素比
                    var backingStoreRatio = this._mainContext.webkitBackingStorePixelRatio ||
                        this._mainContext.mozBackingStorePixelRatio ||
                        this._mainContext.msBackingStorePixelRatio ||
                        this._mainContext.oBackingStorePixelRatio ||
                        this._mainContext.backingStorePixelRatio || 1;
                // canvas的实际渲染倍率
                    ...
                    this._mainContext.scale(0.5,0.5);*/
                ...
                /*   // 屏幕的设备像素比
                   var devicePixelRatio = window.devicePixelRatio || 1;
                // 浏览器在渲染canvas之前存储画布信息的像素比
                   var backingStoreRatio = this._overlayContext.webkitBackingStorePixelRatio ||
                       this._overlayContext.mozBackingStorePixelRatio ||
                       this._overlayContext.msBackingStorePixelRatio ||
                       this._overlayContext.oBackingStorePixelRatio ||
                       this._overlayContext.backingStorePixelRatio || 1;
                // canvas的实际渲染倍率
                   var ratio = devicePixelRatio / backingStoreRatio;
                   /!*  debugger
                     canvas.style.width = canvas.width + 'px';
                     canvas.style.height = canvas.height + 'px';
                      canvas.width = canvas.width * ratio;
                     canvas.height = canvas.height * ratio;*!/
                   this._overlayContext.scale(0.5,0.5);*/
        ...
        value: function updateData(dsName, data) {
            ...
            var dpNames = [".main", ".secondary", ".secondary2"];
            var area = void 0,
                areaName = void 0;
            for (var n in this._areas) {
                area = this._areas[n];
        ...
        key: 'onTouchMove',
        value: function onTouchMove(frameName, x, y, drag) {
            var frame = this.getFrame(frameName);
            if (frame === undefined) return;
            this.setFrameMousePos(frameName, x, y);
            /*this.hideCrossCursor();*/
        ...
        value: function createIndicatorAndRange(areaName, indicName, notLoadSettings) {
            ...
            switch (indicName) {
                ...
                case "StochRSI":
                    ...
                    break;
                case "TREND":
                    indic = new indicators.TRENDIndicator();
                    range = new ranges.Range(areaName);
                    break;
                default:
                    return null;
