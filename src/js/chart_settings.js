...
export class ChartSettings {
    ...
    static init() {
        let _indic_param = {};
        let _name = [ 'MA', 'EMA', 'VOLUME', 'MACD', 'KDJ', 'StochRSI', 'RSI', 'DMI', 'OBV', 'BOLL', 'DMA', 'TRIX', 'BRAR', 'VR', 'EMV', 'WR', 'ROC', 'MTM', 'PSY','TREND'];
    ...
    static load() {
        if (document.cookie.length <= 0)
            return;
        let start = document.cookie.indexOf("chartSettingsJ=");
        if (start < 0)
            return;
    ...
    static save() {
        ...
        document.cookie = "chartSettingsJ=" + escape(JSON.stringify(ChartSettings._data)) +
