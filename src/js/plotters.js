...
export class Plotter extends NamedObject {
    ...
    static createRectangles(context, rects, sign) {
        context.beginPath();
...
export class CandlestickPlotter extends NamedObject {
    ...
    Draw(context) {
        ...
        let coordinates = Kline.instance.data.coordinates;
        let monthCoordinates = Kline.instance.data.monthCoordinates;
        ...
        let colorList  = ['#ffa39e', '#ff9c6e', '#ffc069','#ffc53d','#fadb14','#7cb305','#096dd9','#08979c','#389e0d'];
        ...
        //标线
        let signLine = [];
        let closeNumber = ds.getDataAt(ds._dataItems.length-1).close;
        if(range.toY(closeNumber*0.85) > area.getTop()){
            ...
            function drawLineDash(ctx, x1, x2, y, yn, value) {
                ctx.setLineDash([5,10]);
                ctx.lineWidth = 2;
...
export class RangePlotter extends NamedObject {
    ...
    Draw(context) {
        ...
        if (isMainRange) {
            ...
            if (!area.isChanged() && !range.isUpdated() && areaName!=="frame0.k0.indic0Range")
                return;
        }
        let gradations = range.getGradations();
